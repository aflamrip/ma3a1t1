...
const smartTruncate = (str: string, max: number) => {
  ...
  return (lastSpace > 15 ? truncated.substring(0, lastSpace) : truncated).trim() + '...';
};
...
export const wrapMetadata = (meta: { title: string; description: string; schema: any; ogType: string }) => {
  return {
    ...meta,
    title: smartTruncate(meta.title, 65),
...
export function arabicMovieTemplate(input: SeoItemInput): SeoMetadata {
  ...
  let seoTitle = isWatchPage ? `مشاهدة فيلم ${title} ${year} كامل` : `فيلم ${title} ${year} كامل`;
  let description = isWatchPage ? `مشاهدة وتحميل فيلم ${title} ${year} اون لاين بجودة عالية كامل ${overview}` : `مشاهدة فيلم ${title} اون لاين - ${overview}`;
  ...
  const watchUrl = `${safeSiteUrl}/w/${slug}`;
  const ratingValue = (item.data?.rating || item.data?.vote_average || 8.6).toString();
  const reviewCount = (item.data?.vote_count || 128).toString();
  ...
  const schema: any = {
    ...
    "genre": item.data?.genres || ['عربي', 'سينما'],
    "aggregateRating": {
    ...
    ...(cast.length > 0 && { "actor": cast.map((name: string) => ({ "@type": "Person", "name": name, "url": `${safeSiteUrl}/cast/${encodeURIComponent(name.trim())}` })) }),
    "video": {
      "@type": "VideoObject",
      "name": seoTitle,
      "description": description,
...
export function foreignMovieTemplate(input: SeoItemInput): SeoMetadata {
  ...
  const safeSiteUrl = siteUrl || 'https://ma3ak.top';
  const watchUrl = `${safeSiteUrl}/w/${slug}`;
  const ratingValue = (item.data?.rating || item.data?.vote_average || 8.4).toString();
  const reviewCount = (item.data?.vote_count || 145).toString();
...
export function arabicSeriesTemplate(input: SeoItemInput): SeoMetadata {
  ...
  const director = item.data?.director;
  const cast = Array.isArray(item.data?.cast) ? item.data.cast : [];
  const ratingValue = (item.data?.rating || item.data?.vote_average || 8.8).toString();
  const reviewCount = (item.data?.vote_count || 180).toString();
  ...
  if (isWatchPage && episode) {
    ...
    schema = {
      ...
      "partOfSeries": {
        "@type": "TVSeries",
        "name": title,
        "url": `${safeSiteUrl}/tv/${item.data?.slug}`
      },
      "partOfSeason": {
        "@type": "TVSeason",
        "seasonNumber": sNum,
    ...
    description = `مشاهدة وتحميل جميع حلقات مسلسل ${title} الموسم الاول مسلسلات عربية ${year}`;
...
export function foreignSeriesTemplate(input: SeoItemInput): SeoMetadata {
  ...
  const director = item.data?.director;
  const cast = Array.isArray(item.data?.cast) ? item.data.cast : [];
  const ratingValue = (item.data?.rating || item.data?.vote_average || 8.7).toString();
  const reviewCount = (item.data?.vote_count || 160).toString();
  const sNum = parseInt(season || '1', 10);
